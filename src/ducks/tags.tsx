...
export default function reducer(
	...
	switch (action.type) {
		case TOGGLE_TAG: {
			const updatedTags = getUpdatedTagsAfterSingleToggle(
				state.tags,
				action.id
			);
			...
			const updatedAllTagsSelected = allTagsSelected(updatedTags)
				? true
				: noTagsSelected(updatedTags) ? false : state.allTagsSelected;
			return {
...
function getUpdatedTagsAfterSingleToggle(
	tags: Tag[],
	toggledTagId: number
): Tag[] {
	return tags.map(
		tag =>
			tag.id === toggledTagId ? { ...tag, isSelected: !tag.isSelected } : tag
	);
}
...
function getFilteredProjects(projects: Project[]): Project[] {
	return projects.filter(
		project => project.tags.filter(tag => tag.isSelected).length > 0
	);
}
...
function updateProjectTags(
	...
	let updatedProjectTags = project.tags.map(
		tag =>
